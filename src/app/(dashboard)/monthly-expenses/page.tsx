    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Track where your money goes each month</p>
        </div>
      ...
      {isLoading ? (
        ...
        <>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <Card className="bg-slate-800/50 backdrop-blur-xl ring-1 ring-white/10 border-t-2 border-t-rose-400">
            ...
            <Card className="bg-slate-800/50 backdrop-blur-xl ring-1 ring-white/10 border-t-2 border-t-amber-400">
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">{data.recurringRules.length} active bills</p>
              </CardContent>
            </Card>
            ...
            <Card className={`bg-slate-800/50 backdrop-blur-xl ring-1 ring-white/10 border-t-2 ${data.change > 0 ? "border-t-red-400" : "border-t-green-400"}`}>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">from {formatCurrency(data.lastMonthTotal)}</p>
              </CardContent>
            </Card>
          </div>
              ...
              <CardContent>
                {data.recurringRules.length === 0 ? (
                  <div className="py-8 text-center">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">Add recurring transactions to track your monthly bills</p>
                  </div>
                ) : (
                      ...
                      <div key={rule.id} className="flex items-center justify-between rounded-lg border border-white/5 p-3 hover:bg-white/5 transition-colors">
                        <div>
                          ...
                          <div className="flex gap-2 mt-1">
                            ...
                            <span className="text-xs text-muted-foreground">Next: {format(new Date(rule.nextRun), "MMM d")}</span>
                          </div>
                        </div>
